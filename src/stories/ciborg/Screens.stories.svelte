	...
	>
		<div class="space-y-6">
			...
			<div class="grid grid-cols-3 gap-4">
				{#each [{ label: 'Pending jobs', value: '14' }, { label: 'Workers online', value: '8' }, { label: 'Success rate', value: '98.4%' }] as metric}
					<Card>
						<div class="space-y-1">
...
<Story name="Chats">
	<Screen {sidebarItems} activeNav="Chats" user={{ name: 'John Doe' }}>
		<div class="space-y-5">
			...
			<div class="grid grid-cols-3 gap-4">
				{#each [{ label: 'Open threads', value: '18', note: 'Across coding, review, and ops workflows.' }, { label: 'Waiting on user', value: '5', note: 'Threads paused on approvals, replies, or missing details.' }, { label: 'Project-linked', value: '7', note: 'Optional context, not the default starting point.' }] as metric}
					<Card>
						<div class="space-y-1">
							<div class="text-sm text-[var(--muted-foreground)]">{metric.label}</div>
							<div class="text-3xl font-semibold text-[var(--foreground)]">{metric.value}</div>
			...
			<Card>
				...
				{/snippet}
				<div class="space-y-3">
					{#each [{ title: 'Review the migration plan for the relay artifact cleanup...', type: 'coding' }, { title: 'Summarize the behavioural risks in the shell approval changes...', type: 'review' }, { title: 'Check the flaky image pipeline in infra / worker-images...', type: 'ops' }] as thread}
...
<Story name="Workers">
	<Screen {sidebarItems} activeNav="Workers" user={{ name: 'John Doe' }}>
		<div class="space-y-6">
			...
			/>
			<div class="grid grid-cols-3 gap-4">
				{#each [{ label: 'Running jobs', value: '12' }, { label: 'Queued jobs', value: '34' }, { label: 'Connected workers', value: '8' }] as metric}
					<Card>
						<div class="space-y-1">
							<div class="text-sm text-[var(--muted-foreground)]">{metric.label}</div>
							<div class="text-3xl font-semibold text-[var(--foreground)]">{metric.value}</div>
			...
			<Card>
				...
				{/snippet}
				<div class="space-y-3">
					{#each [{ title: 'worker-eu-central-1 connected from fra1', state: 'Waiting' }, { title: 'worker-us-east-gpu picked up 2 queued jobs', state: 'Busy' }, { title: 'worker-ap-south-1 drained and disconnected', state: 'Ended' }] as event}
						<div
